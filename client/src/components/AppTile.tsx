...
// The maxxing app marks from brand/*.svg, inlined so the apps grid can render
// them at any size without extra requests. Gradient ids are namespaced with
// useId so multiple instances on one page don't clash. Keep the glyphs in
// sync with the masters in brand/ (see brand/README.md).

const GRADIENTS: Record<AppId, [string, string]> = {
  fitness: ['#f34c4c', '#d61f1f'],
  habit: ['#f8763a', '#de4c07'],
  journal: ['#ffa94d', '#e8820e'],
};

function Glyph({ app, gradientId }: { app: AppId; gradientId: string }) {
  switch (app) {
    case 'fitness':
      return (
...
export function AppTile({ app, size = 40 }: { app: AppId; size?: number }) {
  const id = useId();
  const g = `${id}-g`;
  const s = `${id}-s`;
